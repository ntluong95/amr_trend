...
server <- function(input, output, session) {
  ...
  output$adaptive_plot <- renderPlotly({
    ...
    if ("Linear Trend" %in% input$fig2_outcome && nrow(df_cont) > 0) {
      p1 <- ggplot(df_cont, aes(x = RESPONSE, y = change, colour = DPSIR)) +
        ...
        geom_text(
          data = letter_df[which(letter_df[, "plot"] == "change"), ],
          aes(x = 3.8, y = c(1, 1, 1, 1), label = letter2, fontface = "bold"),
          inherit.aes = FALSE
        ) +
        geom_text(
          data = dat_text,
    ...
    if ("Categorical Trend" %in% input$fig2_outcome && nrow(df_bin) > 0) {
      p2 <- ggplot(df_bin, aes(x = factor(sign), y = RESPONSE, fill = DPSIR)) +
        ...
        facet_wrap(~DPSIR, nrow = 1) +
        geom_text(
          data = letter_df[which(letter_df[, "plot"] == "box"), ],
  ...
  output$fig3_plot <- renderPlotly({
    p <- ggplot() +
      ...
      geom_ribbon(
        data = newdataDPSI,
        aes(x = RESPONSE, ymin = dec.propmse, ymax = dec.proppse, fill = DPSIR),
        lwd = 0.75,
        alpha = 0.5
      ) +
      geom_line(
        data = newdataDPSI,
        aes(RESPONSE, dec.prop, colour = DPSIR),
        lwd = 1.2
      ) +
      geom_point(
        data = DRIVERS[DRIVERS$ISO3 %in% unique(all_wide$ISO3), ],
        aes(
          x = RESPONSE,
          y = dec.prop,
          alpha = tot.alpha,
      ...
      xlim(0, 4) +
      xlab("Stated action") +
      ylab("De-escalating ratio") +
      theme_bw() +
      scale_color_manual(
        values = scales::brewer_pal(palette = "Set1")(5)[c(2, 4, 5, 1)]
      ) +
      scale_fill_manual(
        values = scales::brewer_pal(palette = "Set1")(5)[c(2, 4, 5, 1)]
      ) +
  ...
  output$fig4_plot <- renderPlot({
    ...
    p <- ggplot(
      data_to_plot,
      aes(x = mod_names, y = variable_names, fill = rank)
    ) +
      geom_tile() +
      ...
      ylab("Variable Names") +
      xlab("Model Names") +
      ggtitle("Variable Ranks in Each Model") +
      labs(fill = "Rank")
  ...
  output$fig5_plot <- renderPlotly({
    ggplot(
      aniRmmmmALLig.df[
        which(
          !aniRmmmmALLig.df$responseType %in% c("Gmean") &
            aniRmmmmALLig.df$cvar == c("prodValue")
        ),
      ],
      aes(y = responseValue, x = log10(cval), colour = responseType)
    ) +
      geom_point(aes(group = name), alpha = 0.3) +
      theme_bw() +
      geom_smooth(
        data = aniRmmmmALLig.df[
          which(
            !aniRmmmmALLig.df$responseType %in% c("Gmean") &
              aniRmmmmALLig.df$cvar == c("prodValue") &
  ...
  output$governance_syndrome1 <- renderPlotly({
    ...
    if ("Categorical Trend" %in% input$fig6_outcome) {
      plot_category <- ggplot(
        annotation,
        aes(x = DPSIR, y = freq, fill = syndrome)
      ) +
        geom_col() +
        geom_text(
          aes(label = count),
  ...
  data <- reactive({
    ...
    if (
      input$level == "Tier 2" &&
        (input$dpse == "USE" || input$dpse == "RESISTANCE") &&
        !is.null(input$shortname)
    ) {
  ...
  conditional_color_scheme <- function(data_column, group, dpse, level) {
    ...
    if (
      (dpse == "DRIVERS" && level == "Tier 1") ||
        (dpse == "DRIVERS" &&
          (level == "Tier 2" || level == "Tier 3") &&
          group != "Infections")
    ) {
    ...
    if (length(sorted_breaks) > 1) {
      for (i in 1:(length(sorted_breaks) - 1)) {
        color_scheme[[i]] <- list(
          from = sorted_breaks[i],
          to = sorted_breaks[i + 1],
      ...
      color_scheme[[1]] <- list(
        from = min(data_column, na.rm = TRUE),
        to = max(data_column, na.rm = TRUE),
  ...
  green_red_divergent <- function(data_column) {
    ...
    zero_point <- ifelse(
      min_val < 0 && max_val > 0,
      0,
      ifelse(min_val >= 0, min_val, max_val)
    )
  ...
  render_map <- function(column_name) {
    renderHighchart({
      ...
      year_info <- ifelse(
        grepl("x0008", column_name),
        "between 2000-2008",
        "between 2008-2016"
      )

      # Determine which color scheme to use based on the column
      color_scheme <- conditional_color_scheme(
        dpse_data[[column_name]],
        input$group,
        input$dpse,
        input$level
      )
      ...
      hcmap(
        ...
        hc_title(
          text = paste("Map of", input$dpse, "data", year_info, "by country")
        ) %>%
      ...
      hcmap(
        ...
        hc_title(
          text = paste(
            "Map of",
            input$dpse,
            "data difference between two periods by country"
          )
        ) %>%
  ...
  filtered_data <- reactive({
    ...
    governance_begin_after %>%
      filter(income %in% input$income_gov, trend %in% input$trend) # Example filters based on UI inputs
  })
  ...
  output$governance_changes <- renderPlotly({
    ...
    plot_governance <- df %>%
      ...
      ggrepel::geom_text_repel(
        ...
        aes(
          x = ifelse(Year == min(Year), 1 - 0.05, 2 + 0.05),
          label = glue::glue(
            "{name} ({scales::number(value, accuracy = 0.01)})"
          ),
