...
driver_indicators <- data.frame(
  ...
  DescriptionTier3 = c(
    ...
    "UN Prevalence of HIV Total (% of population ages 15-49)",
    "WB Prevalence of HIV Total (% of population ages 15-49)",
    "UN Prevalence of tuberculosis (per 100 000 population)",
    "WB Incidence of tuberculosis (per 100 000 population)",
    ...
    "Haemophilus influenzae type b immunization coverage among 1-year-olds (%)",
...
use_indicators <- data.frame(
  ...
  Tier2 = c(
    "BroadPerTotalABXUse: Use of broad-spectrum antibiotics",
    "NewABXUse: Newly available antibiotics",
    "Total use: Total Antibiotic Use Per Capita"
  ),
  IndicatorComponent = c(
    "Fluoroquinolones, Macrolides,third-generation cephalosporins,co-amoxiclav, clindamycin, oral vancomycin",
...
resistance_indicators <- data.frame(
  ...
  Tier2 = c(
    ...
    "CR: Carbapenem resistance (average)",
    "STR: Streptococcal resistance (average)"
  ),
  IndicatorComponent = c(
    "-",
...
ecology_indicators <- data.frame(
  ...
  DESCRIPTION = c(
    "Purchasing power parity, 2015 base year",
    "Gini index: Inequality – proportion of the lowest 20 % of the national income distribution (2004-2012)",
    "Mean temperature weighted by population density of 1x1 degree grid cells",
  ...
  UNIT = c(
    "Log10 US dollars per capita",
    "Unit free (0:1)",
    "degree C",
    "tonnes per km2",
    "Log10 (population/km2)"
  )
)
...
action_index_table <- data.frame(
  ...
  Topic = c(
    ...
    "AMR Training (Animal Health and Food Production) *",
    "AMR Awareness (Human Health) *",
    "AMR Awareness (Animal Health and Food Production) *",
    ...
    "AMS and Regulation (Animal & Crop) *",
...
# S4 Table -------------------------------------------------------------------------------------------------
governance_syndrome <- import(
  here("data", "S4 Table.xlsx"),
  ...
  mutate(across(
    .col = c(`2016–2017`, `2017–2018`, `2021–2022`, `2022–2023`),
    .fns = as.numeric
  ))
...
# S8 Table -------------------------------------------------------------------------------------------------
global_model_formulas_general <- import(
  here("data", "S8 Table.xlsx"),
  sheet = "General"
) %>%
  tidyr::fill("Global Models with DPSE interaction", .direction = "downup") %>%
  tidyr::fill("Global Models with income interaction", .direction = "downup")

global_model_formulas_hic <- import(
  here("data", "S8 Table.xlsx"),
  sheet = "HIC"
) %>%
  tidyr::fill("Global Models with DPSE interaction", .direction = "downup") %>%
...
global_model_formulas_lmic <- import(
  here("data", "S8 Table.xlsx"),
  sheet = "LMIC"
) %>%
  tidyr::fill("Global Models with DPSE interaction", .direction = "downup") %>%
...
global_model_formulas_binomial <- import(
  here("data", "S8 Table.xlsx"),
  sheet = "Binomial"
) %>%
  tidyr::fill("Global Models with DPSE interaction", .direction = "downup") %>%
...
# Figure S3 ------------------------------------------------------------------------------------------------
best_model_coeffs <- import(
  here("data", "Fig S3-change-coeffs.rds"),
  trust = TRUE
)
best_model_coeffs_point <- import(
  here("data", "Fig S3-change-coeffs-point.rds"),
  trust = TRUE
)
best_model_coeffs_tile <- import(
  here("data", "Fig S3-change-coeffs-tile.rds"),
  trust = TRUE
)


best_model_coeffs_point_binomial <- import(
  here("data", "Fig S3-bin-coeffs-point.rds"),
  trust = TRUE
)
best_model_coeffs_tile_binomial <- import(
  here("data", "Fig S3-bin-coeffs-tile.rds"),
  trust = TRUE
)
...
heatmap_df_point_s6 <- import(
  here("data", "Fig S6_point_supp.rds"),
  trust = TRUE
)

# Figure S2 ------------------------------------------------------------------------------------------------
heatmap_df_main <- import(here("data", "Fig S2_main.rds"), trust = TRUE)
heatmap_df_main_point <- import(
  here("data", "Fig S2_point_main.rds"),
  trust = TRUE
)
heatmap_df_main_tile <- import(
  here("data", "Fig S2_tile_main.rds"),
  trust = TRUE
)


heatmap_df_bin <- import(here("data", "Fig S2_bin.rds"), trust = TRUE)
heatmap_df_bin_point <- import(
  here("data", "Fig S2_point_bin.rds"),
  trust = TRUE
)
...
letter_df <- import(
  here("data", "new", "Main_fig2_letter.df.rds"),
  trust = TRUE
)
...
# >>># 1) reshape to long
# >>>press_long <- data_viz %>%
# >>>  select(name, ISO3, init_resp, end_resp) %>%
# >>>  distinct() %>%
# >>>  pivot_longer(
# >>>    cols = c(init_resp, end_resp),
# >>>    names_to = "year",
# >>>    values_to = "value"
# >>>  ) %>%
# >>>  # rename the year‐labels
# >>>  mutate(year = recode(year, init_resp = "2016", end_resp = "2023"))
...
# >>># now export to SVG:
# >>>ggsave(
# >>>  filename = "amr_action_index_init.svg",
# >>>  plot = press_noorder, # omit this if p is the last plot you printed
# >>>  width = 10, # in inches
# >>>  height = 12, # adjust to suit your aspect
# >>>  device = "svg" # optional when filename ends in .svg
...
# >>>ggsave(
# >>>  filename = "amr_action_index_order_init.svg",
# >>>  plot = press_order, # omit this if p is the last plot you printed
# >>>  width = 10, # in inches
# >>>  height = 12, # adjust to suit your aspect
# >>>  device = "svg" # optional when filename ends in .svg
...
dpsir_shiny <- import(here("data", "new", "data_shiny.rds"), trust = TRUE) %>%
  ...
  mutate(
    change = case_when(
      level == "Tier 3" &
        DPSIR == "DRIVERS" &
        GROUP %in% c("Sanitation", "Vaccines", "Workforce") ~
        change * -1,
      TRUE ~ change
    )
  )
...
governance_begin_after <- import(
  here("data", "new", "Main_fig6_point.rds"),
  trust = TRUE
) %>%
  ...
  filter(n() == 2) %>%
  mutate(
    max_value_year = which.max(value),
